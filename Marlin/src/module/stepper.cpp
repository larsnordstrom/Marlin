...
void Stepper::_calc_bezier_curve_coeffs(const int32_t v0, const int32_t v1, const uint32_t av)
{
  ...
  __asm__ __volatile__(
      ...
      A("sts bezier_C+1, %6") " sts bezier_C+2, %7" /* bezier_C = %7:%6:?? = 10*(v0-v1) [65 cycles worst] */
      : "+r"(r2),
        "+d"(r3),
        "=r"(r4),
        "+r"(r5),
...
FORCE_INLINE int32_t Stepper::_eval_bezier_curve(const uint32_t curr_step)
{
  ...
  __asm__ __volatile(
      ...
      L("2") " clr __zero_reg__" /* C runtime expects r1 = __zero_reg__ = 0 */
      : "+r"(r0),
        "+r"(r1),
        "+r"(r2),
        "+r"(r3),
...
#if ENABLED(LIN_ADVANCE)
    if (!nextAdvanceISR)
      nextAdvanceISR = advance_isr(); // 0 = Do Linear Advance E Stepper pulses
#endif
...
#if ENABLED(LIN_ADVANCE)
    if (nextAdvanceISR != LA_ADV_NEVER)
...
#define Z_MOVE_TEST !!current_block->steps.c
      ...
      {                 // As long as more ISRs are possible...
        max_rate <<= 1; // Try to double the rate
        if (max_rate >= MAX_STEP_ISR_FREQUENCY_1X)
          break;        // Don't exceed the estimated ISR limit
...
// Init Enable Pins - steppers default to disabled.
  ...
  E6_ENABLE_INIT();
  if (!E_ENABLE_ON)
    E6_ENABLE_WRITE(HIGH);
#endif
#if HAS_E7_ENABLE
  E7_ENABLE_INIT();
  if (!E_ENABLE_ON)
    E7_ENABLE_WRITE(HIGH);
#endif
...
// Init Step Pins
  ...
  last_direction_bits = 0 | (INVERT_X_DIR ? _BV(X_AXIS) : 0) | (INVERT_Y_DIR ? _BV(Y_AXIS) : 0) | (INVERT_Z_DIR ? _BV(Z_AXIS) : 0);
...
#define _PULSE_WAIT()                                                                                                     \
  while (EXTRA_CYCLES_BABYSTEP > (uint32_t)(HAL_timer_get_count(PULSE_TIMER_NUM) - pulse_start) * (PULSE_TIMER_PRESCALE)) \
  { /* nada */                                                                                                            \
  }
#else
...
#define BABYSTEP_AXIS(AXIS, INV, DIR)                 \
  do                                                  \
  {                                                   \
...
#define BABYSTEP_CORE(A, B, INV, DIR, ALT)                  \
  do                                                        \
  {                                                         \
    ...
    _APPLY_STEP(B, _INVERT_STEP_PIN(B), true);              \
    DIR_WAIT_BEFORE();                                      \
    _APPLY_DIR(A, old_dir.a);                               \
    _APPLY_DIR(B, old_dir.b);                               \
    DIR_WAIT_AFTER();                                       \
  } while (0)
